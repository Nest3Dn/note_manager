...
def delete_note(notes):
    ...
    search_term = input("Введите имя или заголовок заметки для удаления: ")

    notes_to_remove = []
    for note in notes:
        if note["Имя"].lower() == search_term.lower() or note["Заголовок"].lower() == search_term.lower():
    ...
    if not notes_to_remove:
        ...
        for note in notes_to_remove:
            notes.remove(note)
        print(f"Удалено заметок: {len(notes_to_remove)} ")
    return notes
...
def main():
    notes = []
    while True:
        command = input("Введите 'создать' для добавления, 'удалить' для удаления или 'стоп' для завершения: ").lower()
